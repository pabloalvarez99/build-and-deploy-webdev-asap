...
-- Consolidated from 6 Railway migrations
-- Adapted for Supabase Auth (auth.users)
-- ============================================
...
CREATE INDEX idx_products_laboratory ON products(laboratory);
CREATE INDEX idx_products_laboratory_category ON products(laboratory, category_id);
CREATE INDEX idx_products_therapeutic_action ON products(therapeutic_action);
CREATE INDEX idx_products_prescription_type ON products(prescription_type);
...
CREATE INDEX idx_orders_reservation_expires ON orders(reservation_expires_at) WHERE status = 'reserved';
...
-- ============================================
CREATE TABLE therapeutic_category_mapping (
...
CREATE INDEX idx_therapeutic_mapping_action ON therapeutic_category_mapping(therapeutic_action);
...
-- ============================================
-- 9. RPC FUNCTIONS for filter endpoints
-- ============================================
-- Stock decrement function (used by webhook)
...
-- THERAPEUTIC CATEGORY MAPPING (public read, admin write)
ALTER TABLE therapeutic_category_mapping ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone can read therapeutic mappings"
    ON therapeutic_category_mapping FOR SELECT
    USING (true);

CREATE POLICY "Admins can manage therapeutic mappings"
    ON therapeutic_category_mapping FOR ALL
    USING (is_admin());

-- ============================================
-- 11. SEED DATA: Professional Categories
-- ============================================
...
-- ============================================
-- 12. SEED DATA: Therapeutic Category Mappings
-- ============================================
...
('Paracetamol 500mg x 20', 'paracetamol-500mg-20', 'Analgesico y antipiretico. Caja con 20 comprimidos.', 5.99, 100, (SELECT id FROM categories WHERE slug = 'dolor-fiebre'), 'https://via.placeholder.com/300x300?text=Paracetamol'),
('Ibuprofeno 400mg x 10', 'ibuprofeno-400mg-10', 'Antiinflamatorio no esteroideo. Caja con 10 comprimidos.', 7.50, 80, (SELECT id FROM categories WHERE slug = 'dolor-fiebre'), 'https://via.placeholder.com/300x300?text=Ibuprofeno'),
...
('Omega 3 x 60 capsulas', 'omega-3-60-capsulas', 'Aceite de pescado rico en EPA y DHA. 60 capsulas blandas.', 18.50, 40, (SELECT id FROM categories WHERE slug = 'vitaminas-suplementos'), 'https://via.placeholder.com/300x300?text=Omega3'),
('Shampoo Anticaspa 400ml', 'shampoo-anticaspa-400ml', 'Shampoo medicado para control de caspa.', 15.99, 60, (SELECT id FROM categories WHERE slug = 'higiene-cuidado-personal'), 'https://via.placeholder.com/300x300?text=Shampoo'),
('Crema Hidratante 200ml', 'crema-hidratante-200ml', 'Crema corporal hidratante para piel seca.', 22.00, 45, (SELECT id FROM categories WHERE slug = 'higiene-cuidado-personal'), 'https://via.placeholder.com/300x300?text=Crema'),
('Panales Bebe Talla M x 40', 'panales-bebe-m-40', 'Panales desechables talla M. Paquete de 40 unidades.', 28.99, 30, (SELECT id FROM categories WHERE slug = 'bebes-ninos'), 'https://via.placeholder.com/300x300?text=Panales'),
('Protector Solar FPS 50', 'protector-solar-fps50', 'Protector solar de amplio espectro. 120ml.', 35.00, 25, (SELECT id FROM categories WHERE slug = 'dermatologia'), 'https://via.placeholder.com/300x300?text=Protector');
