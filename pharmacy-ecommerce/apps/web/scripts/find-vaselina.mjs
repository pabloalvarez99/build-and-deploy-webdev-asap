...
const r = await c.query(`
  SELECT p.id, p.name, p.active, p.external_id, p.laboratory,
         COALESCE(array_agg(pb.barcode ORDER BY pb.barcode) FILTER (WHERE pb.barcode IS NOT NULL), ARRAY[]::varchar[]) AS barcodes
  FROM products p
  LEFT JOIN product_barcodes pb ON pb.product_id = p.id
  WHERE p.name ILIKE '%vaselina%'
  GROUP BY p.id ORDER BY p.active DESC, p.name
`);
console.log(`Productos VASELINA: ${r.rows.length}`);
console.table(r.rows.map(x => ({ active: x.active, name: x.name, ext: x.external_id, n_bc: x.barcodes.length, barcodes: x.barcodes.join(', ').slice(0, 100) })));
